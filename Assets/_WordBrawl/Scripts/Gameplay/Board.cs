...
public class Board : Singleton<Board>
{
    ...
    public void NewGame()
    {
        ...
        if (PlayerPrefs.GetInt(GameConstants.PLAYERPREFS_FIRST_LOGIN, 1) == 0)
        {
            return;
        }
        else
        {
            PlayerPrefs.SetInt(GameConstants.PLAYERPREFS_FIRST_LOGIN, 0);
    ...
    public IEnumerator<float> PopAndRefresh()
    {
        ...
        UIManager.Instance.SetButtonInSettingsActive(false);
        UIManager.Instance.IsInteractable = false;

        WordDisplay.Instance.UndisplayWordAndScore();
        GameUIController.Instance.ScorePopUp();
        ScorePopUp.Instance.SetScore(_currentScore);
