...
public class PlayerDataTracker : SingletonPersistent<PlayerDataTracker>
{
    ...
    {
        _bestWordOfAllTime = PlayerPrefs.GetString(GameConstants.PLAYERPREFS_BEST_WORD_OF_ALL_TIME, "");
        _bestWordScore = PlayerPrefs.GetInt(GameConstants.PLAYERPREFS_BEST_WORD_SCORE, 0);
        _wins = PlayerPrefs.GetInt(GameConstants.PLAYERPREFS_WINS, 0);
        _losses = PlayerPrefs.GetInt(GameConstants.PLAYERPREFS_LOSSES, 0);

        if (PlayerPrefs.HasKey(GameConstants.PLAYERPREFS_POWERUP_USAGE))
        {
            var jsonData = PlayerPrefs.GetString(GameConstants.PLAYERPREFS_POWERUP_USAGE);
    ...
    {
        if (_powerUpUsage.Count == 0) return "";
    ...
    public void LogBestWord(string word, int score)
    {
        if (score > _bestWordScore)
        {
            ...
            PlayerPrefs.SetString(GameConstants.PLAYERPREFS_BEST_WORD_OF_ALL_TIME, _bestWordOfAllTime);
        }
    }
    ...
    public void LogBattleResult(bool isPlayerWon, string word, int score)
    {
        ...
        {
            _wins++;
            PlayerPrefs.SetInt(GameConstants.PLAYERPREFS_WINS, _wins);
        }
        else
        {
            _losses++;
            PlayerPrefs.SetInt(GameConstants.PLAYERPREFS_LOSSES, _losses);
        }
    ...
    private void SavePowerUpUsage()
    {
        ...
        PlayerPrefs.SetString(GameConstants.PLAYERPREFS_POWERUP_USAGE, jsonData);
        PlayerPrefs.Save();
    }
